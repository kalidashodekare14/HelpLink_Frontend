...
} from "@/state/services/profileService/profileService";
...
const ProfileComponent = () => {
  ...
  const {
    ...
    error,
  } = useProfileInfoQuery(session?.user?.email);
  const profileData = profileInfo?.data;
  // Profile Info Update API
  const [profleInfoUpdate, { isLoading: updateLoading, error: updateError }] =
    useProfileInfoUpdateMutation();
  // Profile Image Upload API
  const [profileImageUpload, { isLoading: pImageLoading, error: pImageError }] =
    useProfileImageUploadMutation();
  // Image loading
  ...
  const onSubmit: SubmitHandler<Inputs> = async (data) => {
    try {
      ...
      const result = await profleInfoUpdate({
